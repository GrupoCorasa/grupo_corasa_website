<?php
/**
 * Herramientas de Corte landing — contact form endpoint.
 *
...
const FORM_SOURCE = '/shivelybros-gc/herramientas-de-corte/index.html';
...
if (!is_readable($configPath)) {
    error_log('[shively-contact:herramientas-de-corte] config.php missing or unreadable at ' . $configPath);
...
if ($mailTo === []) {
    error_log('[shively-contact:herramientas-de-corte] MAIL_TO resolved to zero valid recipients');
    http_response_code(500);
    echo json_encode(['ok' => false, 'error' => 'Configuración del servidor incompleta.']);
    exit;
}
...
try {
    ...
    if ($mail->SMTPDebug > 0) {
        $mail->Debugoutput = static function ($str) { error_log('[shively-contact:herramientas-de-corte][smtp] ' . $str); };
    }
    $mail->Timeout = 15;
    ...
    $mail->Subject = sprintf('[Sitio web · Herramientas de Corte] Nuevo contacto — %s — %s', $empresa, $servicio);
    $mail->isHTML(true);
    ...
    error_log('[shively-contact:herramientas-de-corte] PHPMailer error: ' . $mail->ErrorInfo);
    ...
    error_log('[shively-contact:herramientas-de-corte] Unexpected error: ' . $e->getMessage());
    http_response_code(500);
...
function verifyRecaptcha(string $token, array $cfg): bool
{
    ...
    if ($secret === '' || strncmp($secret, 'TODO_', 5) === 0) {
        error_log('[shively-contact:herramientas-de-corte] reCAPTCHA secret key not configured');
        return false;
    }
    ...
    if ($body === false || $httpCode !== 200) {
        error_log('[shively-contact:herramientas-de-corte] reCAPTCHA HTTP failure: code=' . $httpCode . ' err=' . $curlErr);
        return false;
    }
    ...
    if (!is_array($data) || empty($data['success'])) {
        ...
        error_log('[shively-contact:herramientas-de-corte] reCAPTCHA rejected: ' . $codes);
        return false;
    }

    $expectedAction = $cfg['RECAPTCHA_ACTION'] ?? 'contact_submit';
    if (($data['action'] ?? '') !== $expectedAction) {
        error_log('[shively-contact:herramientas-de-corte] reCAPTCHA action mismatch: got=' . ($data['action'] ?? '(none)'));
        return false;
    }
    ...
    if ($score < $minScore) {
        error_log(sprintf('[shively-contact:herramientas-de-corte] reCAPTCHA low score: %.2f < %.2f', $score, $minScore));
        return false;
    }
...
function buildHtmlBody(string $nombre, string $empresa, string $cargo, string $email, string $telefono, string $servicio, string $requerimiento): string
{
    $rows = [
        ...
        ['Origen',                'Landing Herramientas de Corte'],
    ];
    $rowsHtml = '';
    ...
    return '<!doctype html><html lang="es"><body style="font-family:Arial,sans-serif;color:#222;line-height:1.6;">'
         ...
         . '<p style="margin:0 0 18px;color:#666;">Herramientas de Corte · Shively Bros · grupocorasa.mx</p>'
...
{
    return "Nuevo contacto desde el sitio web — Herramientas de Corte · Shively Bros\n"
         . str_repeat('-', 50) . "\n"
         . "Nombre:               $nombre\n"
         . "Empresa:              $empresa\n"
         . "Cargo:                $cargo\n"
